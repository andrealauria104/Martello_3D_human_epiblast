...
# paths ---
path_metadata = "data/experiment_2_untreated/metadata_full.txt"
path_counts = "data/experiment_2_untreated/raw_counts.csv.gz"
path_gene_info = "data/gene_info.gencode.hsapiens.32.primary_assembly.annotation.txt.gz"

# output dirs ---
path_results  = "results/experiment_2_untreated"

if(!dir.exists(path_results)) dir.create(path_results, recursive = T)

# Global params ---
metadata_features <- c("Plate","Time")
...
sce <- sce[,-which(sce$assigned_reads==0)]
sce <- sce[,-grep("MBULK|EMPTY|MINIBULK",colnames(sce))]
sce <- sce[,!sce$Plate%in%c("C147","C153","C153bis","C155")]
...
qc.assigned <- sce$Assigned_rate_total < 0
qc.lib <- qc_df$sum < 1e4#1e5
qc.nexprs <- qc_df$detected < 5e2#5e3
# qc.spike <- qc_df$altexps_ERCC_percent > 10
qc.mito <- qc_df$subsets_Mito_percent > 90 #10
